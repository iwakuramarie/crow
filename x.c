...
xhints(void)
{
	XClassHint class = {opt_name ? opt_name : "crow",
	                    opt_class ? opt_class : "Crow"};
...
xdrawcursor(int cx, int cy, Glyph g, int ox, int oy, Glyph og, Line line, int len)
{
	...
	if (IS_SET(MODE_FOCUSED)) {
		switch (win.cursor) {
		case 7: /* crow extension: snowman (U+2603) */
			g.u = 0x2603;
...
resource_load(XrmDatabase db, char *name, enum resource_type rtype, void *dst)
{
	...
	snprintf(fullname, sizeof(fullname), "%s.%s",
			opt_name ? opt_name : "crow", name);
	snprintf(fullclass, sizeof(fullclass), "%s.%s",
			opt_class ? opt_class : "Crow", name);
...
run:
	...
	if (!opt_title)
		opt_title = (opt_line || !opt_cmd) ? "crow" : opt_cmd[0];
